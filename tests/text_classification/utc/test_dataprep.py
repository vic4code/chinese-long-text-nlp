...
from .conftest import is_data_valid_for_utc_input_format
...
class TestDistinctDataLength:
    def test_convert_success_when_normal_case(self, label_studio_args):
        ...
        assert is_data_valid_for_utc_input_format(data_path=train_path)
        assert is_data_valid_for_utc_input_format(data_path=dev_path)
        assert is_data_valid_for_utc_input_format(data_path=test_path)
    ...
    def test_convert_success_when_only_one_data(self, label_studio_args):
        ...
        # Then
        train_path, test_path = (os.path.join(label_studio_args["save_dir"], data_type) for data_type in ["train.txt", "test.txt"])
        assert os.path.exists(train_path)
        assert os.path.exists(test_path)
        assert is_data_valid_for_utc_input_format(data_path=train_path)
        assert is_data_valid_for_utc_input_format(data_path=test_path)
    ...
    def test_convert_fail_when_no_data_then_raise_valueerror(self, label_studio_args):
        # Given
        label_studio_args["label_studio_file"] = "./tests/text_classification/data/label_studio/utc/no_data_test.json"
    ...
    def teardown_method(self):
        ...
        clean_path = "./data/information_extraction/model_input_data/"
