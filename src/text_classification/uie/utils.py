...
class RuleBasedProcessor:
    """
    Rule based text processor to get the description segments related to schema prompts defined in uie.
    """
    ...
    def postprocessing(self, raw_text, uie_output, schema):
        ...
        for label_type in schema:
            if uie_output[0].get(label_type) is None:
                ...
                while True:
                    ...
                    for result in result_list:
                        if result["end"] - result["start"] > max_content_len:
                            logger.warning(
                                "result['end'] - result ['start'] exceeds max_content_len, "
                                "which will result in no valid instance being returned"
                            )
        ...
        for items in raw_examples:
            ...
            if "data" in items.keys():
                ...
                if not relation_mode:
                    ...
                    # e.g. {"data": "", "label": {"relations": [ {"id": 0, "start_offset": 0, "end_offset": 6, "label": "ORG"}, ... ],
                ...
                if "label" in items.keys():
                    ...
                    # e.g. {"text": "", "relations": [ {"id": 0, "start_offset": 0, "end_offset": 6, "label": "ORG"}, ... ],
                    #       "entities": [ {"id": 0, "from_id": 0, "to_id": 1, "type": "foundedAt"}, ... ]}
...
def convert_example(example, tokenizer, max_seq_len, multilingual=False, dynamic_max_length: Optional[List[int]] = None):
    """
    example: {
        title
        prompt
        ...
        for key in informations:
            for each_information in informations[key]:
                if each_information["probability"] > threshold:
                    if each_information["start"] not in test_unique["start"] and each_information["end"] not in test_unique["end"]:
...
def filter_text(
    ...
    """
    Maximize the utilization of max content text length space.
    """
    ...
    for u, window in enumerate(sliding_windows):
        ...
        if window["start"] < last_end:
            ...
            if dynamic_adjust_length:
                ...
                waste_length += (
                    int(np.round((last_end - window["start"]) / remain_windows)) if remain_windows > 0 else (last_end - window["start"])
                )

                if u == len(sliding_windows) - 1:
                    window["end"] = (
                        window["end"] + int(np.round(waste_length / 2)) if window["end"] + waste_length < raw_text_len else raw_text_len
                    )
