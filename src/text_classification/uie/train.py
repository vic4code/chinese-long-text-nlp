...
from paddlenlp.trainer import CompressionArguments, Trainer, get_last_checkpoint
...
class DataArguments:
    ...
    train_path: str = field(
        default="./data/modeling/text_classification/model_train/uie/train.txt",
        metadata={"help": "The name of the train.txt file."},
    )
    dev_path: str = field(
        default="./data/modeling/text_classification/model_train/uie/dev.txt",
        metadata={"help": "The name of the dev.txt file."},
    )
...
def finetune(data_args, training_args, model_args) -> None:
    ...
    logger.warning(
        ...
        f"world_size: {training_args.world_size}, distributed training: {bool(training_args.local_rank != -1)}, "
    ...
    trainer.optimizer = paddle.optimizer.AdamW(learning_rate=training_args.learning_rate, parameters=model.parameters(), weight_decay=0.3)
...
def main(args: dict):
    ...
    model_args, data_args, training_args = (
        ...
        CompressionArguments(**args["training_args"]),
    )
