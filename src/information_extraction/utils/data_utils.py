            ...
            while len(content) > 0:
                ...
                for each_result in current_content_result:
                    ...
                    if adjust_data != true_data:
                        logger.error(
                            f"each_result: {each_result}. Possible Reason: 1. Dirty data: use is_regularize=True in convert section. "
                            "2. Sorted Problem (if index is negative) : result must be sorted by start index."
                        )
