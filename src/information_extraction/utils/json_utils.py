...
from judge.information_extraction import COLUMN_NAME_OF_JSON_CONTENT
...
def convert_format(dataset: List[dict], entity_type: List[str], is_shuffle: bool = True) -> List[dict]:
    ...
    Raises:
        ValueError: 任務格式錯誤（此邏輯程式只處理 NER 任務，有 Relation 標籤不處理）。
    ...
    for data in dataset:
        uie_format = {
            output_type: {
                "content": data["data"][COLUMN_NAME_OF_JSON_CONTENT],
                "result_list": [],
                "prompt": output_type,
            }
...
def read_json(json_file: str) -> None:
    ...
    if os.path.exists(json_file):
        ...
        with open(json_file, "r", encoding="utf-8") as f:
            infile = f.read()
            if len(infile) == 0:
                raise ValueError(f"Data not found in file: {json_file}")
            all_content = json.loads(infile)
        return all_content if isinstance(all_content, list) else [all_content]
    else:
...
def regularize_content(
    ...
    Args:
        single_json (str):
            A JSON file which wants to be regularized.
            This file must be the same format with label studio output,
            and have the labels of NER tasks such as 'start' and 'end' index.

        regularize_text ((Optional[List[str]], optional):
            List of the special tokens.
            Each string in the list must be only one character (len(TOKEN) == 1).
            Defaults to ["\n", " ", "\u3000"].

        special_result_case (Optional[List[str]], optional):
            Other special case which cannot be regularize in regularize_text.
            Defaults to [r"\n"].
    ...
    if len(single_json["annotations"][0]["result"]) > 0:
        result_index = []
        # sorted result
        single_json["annotations"][0]["result"] = sorted(single_json["annotations"][0]["result"], key=lambda item: item["value"]["start"])
        for i in single_json["annotations"][0]["result"]:
            result_index.append(i["value"]["start"])
            result_index.append(i["value"]["end"] - 1)
        ...
        for i, char in enumerate(single_json["data"][COLUMN_NAME_OF_JSON_CONTENT]):
        ...
        for i in range(len(single_json["annotations"][0]["result"])):
            ...
            single_json["annotations"][0]["result"][i]["value"]["end"] = result_index[result_index_pointer + 1] + 1
    ...
    single_json["data"][COLUMN_NAME_OF_JSON_CONTENT] = re.sub(pattern, "", single_json["data"][COLUMN_NAME_OF_JSON_CONTENT])
    return single_json
...
def regularize_json_file(
    ...
    Args:
        json_file (str):
            A JSON file path which contains several JSONs which want to be regularized.

        out_variable (bool):
            Defaults to False. If the output is a python varialble not write a file.

        output_path (str, optional):
            The path of regularized JSON. Defaults to "./".

        regularize_text (Optional[List[str]], optional):
            List of the special tokens. Each string in the list must be only one character (len(TOKEN) == 1).
            Defaults to ["\n", " ", "\u3000"].

        special_result_case (Optional[List[str]], optional):
            Other special case which cannot be regularize in regularize_text. Defaults to [r"\n"].
    """

    if not os.path.exists(json_file):
        raise ValueError(f"Label studio file not found in {json_file}. Please input the correct path of label studio file.")
    ...
    def test_regularized_data(regularized_data: dict) -> bool:
        for i in range(len(regularized_data["annotations"][0]["result"])):
            ...
            adjusted_data = regularized_data["data"][COLUMN_NAME_OF_JSON_CONTENT][start:end]
            true_data = regularized_data["annotations"][0]["result"][i]["value"]["text"]
            if adjusted_data != true_data:
                raise ConvertingError(f"adjusted_data: {adjusted_data} is not equal to true_data: {true_data}. start:end = {start}:{end}")

    for i in regularize_text:
        if len(i) != 1:
            raise ValueError("Default special token in regularize_text takes only 1 char!")

    logger.info("Start regularize ...")

    json_list = read_json(json_file)
    result_list = []

    for each_json in json_list:
        regularized_data = regularize_content(each_json, regularize_text=regularize_text, special_result_case=special_result_case)
        test_regularized_data(regularized_data)
        result_list.append(regularized_data)

    logger.info("Finish regularize data...")
