...
from . import ENTITY_TYPE, REGULARIZED_TOKEN, logger
from .utils.json_utils import convert_format, regularize_json_file, set_seed, shuffle_data
...
def split_labelstudio(
    ...
    if len(split_ratio) != 3:
        raise ValueError(f"Length Error in split_ratio: {len(split_ratio)}. Only accept len(split_ratio)==3 for splits.")
    ...
    if labelstudio_list:
        ...
        if not os.path.exists(labelstudio_file):
            raise ValueError(f"Label studio file not found in {labelstudio_file}. " + "Please input the correct path of label studio file.")

        with open(labelstudio_file, "r", encoding="utf-8") as f:
            infile = f.read()
            if len(infile) == 0:
                raise ValueError(f"Data not found in file: {labelstudio_file}")
...
def main(args: dict):
    ...
    if args["is_regularize_data"]:
        regularized_result = regularize_json_file(json_file=args["labelstudio_file"], out_variable=True, regularize_text=REGULARIZED_TOKEN)
